...
def start_track(driver):
    ...
    try:
        ...
        steps.click_it(driver, browser, driver.find_element_by_class_name('action'),
                       'Clicking on first "Start Free Track" button.')

        driver.execute_script("window.scrollTo(0, document.body.scrollHeight);")

        steps.click_it(driver, browser,
                       driver.find_element_by_class_name('bottom-panel').find_element_by_tag_name('button'),
                       'Clicking on Start Track button.')
